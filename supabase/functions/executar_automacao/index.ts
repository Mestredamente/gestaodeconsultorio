...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!record || !record.usuario_id) {
      return new Response(JSON.stringify({ message: 'Payload inválido ou sem usuario_id' }), {
        headers: corsHeaders,
      })
    }
    ...
    if (!matchedGatilho) {
      return new Response(JSON.stringify({ message: 'Nenhum gatilho correspondente' }), {
        headers: corsHeaders,
      })
    }
    ...
    if (fluxosError || !fluxos || fluxos.length === 0) {
      return new Response(JSON.stringify({ message: 'Nenhum fluxo ativo encontrado' }), {
        headers: corsHeaders,
      })
    }
    ...
    return new Response(JSON.stringify({ success: true, processed: executados }), {
      headers: corsHeaders,
    })
  } catch (error: any) {

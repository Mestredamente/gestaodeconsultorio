    ...
    Tables: {
      ...
      atividades: {
        Row: {
          ...
          sentimento_analise: string | null
      ...
      lgpd_logs: {
        ...
        Relationships: [
          {
            foreignKeyName: "lgpd_logs_clinic_id_fkey"
            columns: ["clinic_id"]
            isOneToOne: false
            referencedRelation: "usuarios"
            referencedColumns: ["id"]
          ...
          {
            foreignKeyName: "lgpd_logs_patient_id_fkey"
            columns: ["patient_id"]
            isOneToOne: false
            referencedRelation: "pacientes"
            referencedColumns: ["id"]
          ...
          {
            foreignKeyName: "lgpd_logs_performed_by_fkey"
      ...
      nfse_records: {
        ...
        Relationships: [
          ...
          {
            foreignKeyName: "nfse_records_payment_id_fkey"
            columns: ["payment_id"]
            isOneToOne: false
...
//   sentimento_score: integer (nullable)
//   sentimento_analise: text (nullable)
// Table: audit_log
//   id: uuid (not null, default: gen_random_uuid())
//   user_id: uuid (nullable)
...
//   nfse_number: text (nullable)
//   status: text (not null, default: 'processing'::text)
//   xml_data: text (nullable)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
// Table: notificacoes
//   id: uuid (not null, default: gen_random_uuid())
...
// Table: lgpd_logs
//   FOREIGN KEY lgpd_logs_clinic_id_fkey: FOREIGN KEY (clinic_id) REFERENCES usuarios(id) ON DELETE CASCADE
//   FOREIGN KEY lgpd_logs_patient_id_fkey: FOREIGN KEY (patient_id) REFERENCES pacientes(id) ON DELETE CASCADE
//   FOREIGN KEY lgpd_logs_performed_by_fkey: FOREIGN KEY (performed_by) REFERENCES usuarios(id) ON DELETE SET NULL
//   PRIMARY KEY lgpd_logs_pkey: PRIMARY KEY (id)
// Table: lista_espera
...
// Table: nfse_records
//   FOREIGN KEY nfse_records_clinic_id_fkey: FOREIGN KEY (clinic_id) REFERENCES usuarios(id) ON DELETE CASCADE
//   FOREIGN KEY nfse_records_payment_id_fkey: FOREIGN KEY (payment_id) REFERENCES payments(id) ON DELETE SET NULL
//   PRIMARY KEY nfse_records_pkey: PRIMARY KEY (id)
// Table: notificacoes
...
//     WITH CHECK: (usuario_id = auth.uid())
// Table: lgpd_logs
//   Policy "lgpd_logs_policy" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (clinic_id IN ( SELECT usuarios.id    FROM usuarios   WHERE (COALESCE(usuarios.parent_id, usuarios.id) = get_tenant_id())))
// Table: lista_espera
...
//     WITH CHECK: (usuario_id = auth.uid())
// Table: nfse_records
//   Policy "nfse_records_policy" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (clinic_id IN ( SELECT usuarios.id    FROM usuarios   WHERE (COALESCE(usuarios.parent_id, usuarios.id) = get_tenant_id())))
// Table: notificacoes
...
// Table: pacientes
//   trg_log_lgpd_pacientes: CREATE TRIGGER trg_log_lgpd_pacientes AFTER INSERT OR DELETE OR UPDATE ON public.pacientes FOR EACH ROW EXECUTE FUNCTION log_lgpd_action()
// Table: parametros_financeiros

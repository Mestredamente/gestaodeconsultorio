...
import { Loader2, Video, FileText, PhoneOff, Mic, MicOff, VideoOff, CheckCircle, CalendarPlus } from 'lucide-react'
...
export default function VirtualRoom() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'prontuarios', filter: `paciente_id=eq.${activeSession.paciente_id}` }, (payload) => {
  ...
  const handleNotesChange = (val: string) => {
    ...
    typingTimeoutRef.current = setTimeout(async () => {
      if (activeSession) {
        await supabase.from('prontuarios').update({ nova_nota: val }).eq('paciente_id', activeSession.paciente_id)
      }
  ...
  const handleCompileNotes = async () => {
    ...
    const { data: pront } = await supabase.from('prontuarios').select('historico_sessoes').eq('paciente_id', activeSession.paciente_id).single()
  ...
  useEffect(() => {
    const fetchTimes = async () => {
      ...
      const free = allSlots.filter(s => !occupied.includes(s)).map(s => `${String(s).padStart(2, '0')}:00`)
  ...
  const confirmNextSession = async () => {
    ...
    try {
      ...
      const patient = Array.isArray(activeSession.pacientes) ? activeSession.pacientes[0] : activeSession.pacientes

      const { data: newApt, error } = await supabase.from('agendamentos').insert({
        usuario_id: user.id,
  ...
  if (loading) return <div className="flex items-center justify-center h-[80vh]"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>
      ...
      <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-500">Selecione uma sessão agendada para hoje para iniciar o atendimento.</p>
          </div>
        </div>
        ...
        {appointments.length === 0 ? (
          <div className="text-center py-20 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 mt-1">Você não possui agendamentos marcados para o dia de hoje.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {appointments.map(apt => {
              const p = Array.isArray(apt.pacientes) ? apt.pacientes[0] : apt.pacientes
              const time = new Date(apt.data_hora).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
              return (
                <Card key={apt.id} className="rounded-2xl border-slate-200 hover:shadow-md transition-shadow">
                  <CardContent className="p-6 flex flex-col justify-between h-full gap-6">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">{apt.is_online ? 'Consulta Online' : 'Consulta Presencial'}</p>
                      </div>
                    ...
                    <Button onClick={() => joinSession(apt)} className="w-full gap-2 rounded-xl h-11 bg-primary hover:bg-primary/90">
    ...
    <div className="h-[calc(100vh-80px)] flex flex-col bg-slate-950 rounded-3xl overflow-hidden shadow-2xl border border-slate-800 animate-fade-in relative mt-2 mb-6 max-w-[1600px] mx-auto">
      ...
      <div className="h-16 px-6 flex items-center justify-between bg-slate-900/80 backdrop-blur-md border-b border-slate-800 absolute top-0 w-full z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-white font-medium">Sessão com <strong className="font-bold">{patient?.nome}</strong></span>
        ...
        <Sheet open={isNotesOpen} onOpenChange={setIsNotesOpen}>
          <SheetTrigger asChild>
            <Button variant="secondary" className="gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border-0 h-9 rounded-full px-4">
          ...
          <SheetContent className="w-full sm:max-w-md border-l border-slate-200 flex flex-col bg-white">
            <SheetHeader className="pb-4 border-b border-slate-100 shrink-0">
              <SheetTitle className="text-xl font-bold flex items-center justify-between">
                <span className="flex items-center gap-2"><FileText className="w-5 h-5 text-primary" /> Notas da Sessão</span>
                {isSavingNotes ? (
                  <span className="text-xs text-slate-400 flex items-center gap-1 font-normal"><Loader2 className="w-3 h-3 animate-spin" /> Salvando...</span>
                ) : (
                  <span className="text-xs text-emerald-600 flex items-center gap-1 font-normal"><CheckCircle className="w-3 h-3" /> Salvo</span>
                )}
              </SheetTitle>
              <SheetDescription>
            ...
            <div className="pt-4 border-t border-slate-100 shrink-0">
              <Button onClick={handleCompileNotes} className="w-full h-11 rounded-xl gap-2 bg-slate-900 hover:bg-slate-800">
      ...
      <div className="flex-1 relative flex items-center justify-center bg-slate-900">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <img src={`https://img.usecurling.com/ppl/large?gender=female&seed=${activeSession.id}`} alt="Patient" className="w-full h-full object-cover opacity-60 mix-blend-luminosity" />
        ...
        <div className="absolute bottom-24 right-6 w-48 h-64 bg-slate-800 rounded-2xl border-2 border-slate-700 overflow-hidden shadow-xl">
           <img src={`https://img.usecurling.com/ppl/thumbnail?gender=male&seed=${user?.id}`} alt="You" className="w-full h-full object-cover" />
           <div className="absolute bottom-2 left-2 text-white bg-black/40 px-2 py-1 rounded text-xs backdrop-blur-sm">Você</div>
        </div>
      </div>
      ...
      <div className="h-20 bg-slate-950 flex items-center justify-center gap-4 px-6 border-t border-slate-800">
        <Button 
          ...
          className={cn("w-12 h-12 rounded-full border-0", micOn ? "bg-slate-800 text-white hover:bg-slate-700" : "bg-red-500 text-white hover:bg-red-600")}
        >
        ...
        <Button 
          ...
          className={cn("w-12 h-12 rounded-full border-0", videoOn ? "bg-slate-800 text-white hover:bg-slate-700" : "bg-red-500 text-white hover:bg-red-600")}
        >
        ...
        <Dialog open={isFinishModalOpen} onOpenChange={setIsFinishModalOpen}>
          ...
          <DialogContent className="sm:max-w-md rounded-[2rem]">
            ...
            <div className="p-6 pt-2 flex flex-col sm:flex-row gap-3">
              <Button variant="outline" className="flex-1 h-12 rounded-xl text-base" onClick={skipScheduling}>
                Pular
              </Button>
              <Button className="flex-1 h-12 rounded-xl text-base gap-2" onClick={confirmNextSession} disabled={!nextDate || !nextTime || isScheduling}>
